...
export function registerConnectorTools(server: McpServer, run: ToolRunner): void {
  server.registerTool(
    ...
    async ({ action, connector_id }, extra) =>
      run(extra, (client) => client.listConnectors({ action, connector_id }))
  );
  ...
  server.registerTool(
    ...
    async ({ connector_id }, extra) =>
      run(extra, (client) => client.getConnectorOAuth2Required(connector_id))
  );
}
