...
| `tealfabric_download_document` | Download a file (binary returned as base64) | `file_path`, `tenant_id` (optional) |
...
| `tealfabric_download_document` | GET | `/api/v1/documents?action=download&file_path={path}` (+ optional `tenant_id`) |
