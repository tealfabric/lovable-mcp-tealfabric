...
export function registerDocumentTools(server: McpServer, run: ToolRunner): void {
  server.registerTool(
  ...
  server.registerTool(
    ...
    async ({ file_path, tenant_id }, extra) =>
      run(extra, (client) => client.getDocumentMetadata({ file_path, tenant_id }))
  );
  ...
  server.registerTool(
    "tealfabric_upload_document",
    {
      ...
      inputSchema: z.object({
        destination_path: z
          .string()
          .describe(
            "Directory path on the server (without filename). File keeps its original name (e.g. packages/ or packages/reports/)"
          ),
        file_path: z
          .string()
    ...
    async ({ destination_path, file_path, tenant_id }, extra) =>
      run(extra, (client) => client.uploadDocument({ destination_path, file_path, tenant_id }))
  );
  ...
  server.registerTool(
    ...
    async ({ old_path, new_path, tenant_id }, extra) =>
      run(extra, (client) => client.moveDocument({ old_path, new_path, tenant_id }))
  );
